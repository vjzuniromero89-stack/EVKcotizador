...
.catalog-card{cursor:pointer}.catalog-detail-backdrop{position:fixed;inset:0;background:rgba(3,28,46,.72);z-index:200;display:flex;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(4px)}.catalog-detail{width:min(980px,96vw);max-height:92vh;overflow:auto;background:#fff;border-radius:24px;box-shadow:0 28px 90px rgba(0,0,0,.28);position:relative}.catalog-detail-close{position:absolute;right:16px;top:16px;width:42px;height:42px;border-radius:50%;border:0;background:#fff;display:grid;place-items:center;box-shadow:0 5px 18px rgba(0,0,0,.16);z-index:2}.catalog-detail-head{display:grid;grid-template-columns:minmax(280px,.9fr) 1.1fr;gap:0}.catalog-detail-photo{min-height:390px;background:linear-gradient(135deg,#eef5f8,#dceaf1);display:flex;align-items:center;justify-content:center}.catalog-detail-photo img{width:100%;height:100%;max-height:470px;object-fit:contain;padding:24px}.catalog-detail-info{padding:36px}.catalog-detail-info h2{font-family:'Barlow Condensed',sans-serif;font-size:40px;line-height:1.05;margin:6px 0 14px;color:#092f49}.catalog-detail-desc{white-space:pre-wrap;color:#5c7180;line-height:1.6;font-size:15px;max-height:230px;overflow:auto}.catalog-detail-metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:22px}.catalog-detail-metric{background:#f4f8fb;border:1px solid #d7e4eb;border-radius:14px;padding:14px}.catalog-detail-metric span{display:block;font-size:12px;color:#6e8390;margin-bottom:5px}.catalog-detail-metric strong{font-family:'Barlow Condensed',sans-serif;font-size:24px;color:#0b2d45}.catalog-detail-price{margin-top:16px;background:linear-gradient(135deg,#073b5e,#0b7daf);color:#fff;border-radius:16px;padding:18px 20px;display:flex;justify-content:space-between;align-items:center}.catalog-detail-price span{font-size:14px;color:#d8edf7}.catalog-detail-price strong{font-family:'Barlow Condensed',sans-serif;font-size:34px}.catalog-click-hint{font-size:12px;color:#0b7daf;margin-top:10px;font-weight:700}
@media(max-width:760px){.catalog-hero{min-height:390px;padding:32px 20px}.catalog-logo{height:90px}.catalog-toolbar{align-items:stretch;flex-direction:column}.catalog-search{width:100%}.catalog-grid{grid-template-columns:1fr}.catalog-photo{height:250px}.catalog-footer{flex-direction:column;text-align:center}.catalog-footer img{width:190px}.catalog-detail-head{grid-template-columns:1fr}.catalog-detail-photo{min-height:260px}.catalog-detail-info{padding:24px}.catalog-detail-info h2{font-size:32px}.catalog-detail-metrics{grid-template-columns:1fr 1fr}}
`;
...
function ProductosView({ productos, guardarProductos, avisar, empresa, guardarEmpresa }) {
  ...
  async function submit() {
    ...
    if (num(form.precioVenta) <= 0) { setError("Ingresa el precio de venta. Todos los productos del catálogo deben tener precio de venta."); return; }
  ...
  return <div className={embedded?"catalog-admin-wrap":"catalog-public-page"}>
    ...
    <section className="catalog-body">
      ...
      {cargandoCatalogo?<div className="catalog-empty">Cargando catálogo…</div>:errorCatalogo?<div className="catalog-empty">{errorCatalogo}</div>:filtrados.length===0?<div className="catalog-empty">No encontramos productos con esa búsqueda.</div>:<div className="catalog-grid">{filtrados.map(p=><article key={p.id||p.codigo} className="catalog-card" onClick={()=>setSeleccionado(p)} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setSeleccionado(p)}}}>
        ...
        <div className="catalog-card-body"><div className="catalog-code">{p.codigo}</div><h3>{p.nombre}</h3>{p.descripcion&&<p className="catalog-desc">{p.descripcion}</p>}<div className="catalog-price-row"><span>Precio</span><strong>{num(p.precio_venta)>0?money(p.precio_venta):"Precio pendiente"}</strong></div><div style={{fontSize:12,color:'#6B7B86',marginTop:-6,marginBottom:10}}>Precio puesto en Managua.</div><button type="button" className="btn" style={{width:'100%',background:'#1FA463',color:'#fff',marginBottom:10,fontWeight:800}} onClick={(e)=>{e.stopPropagation();setSeleccionado(p)}}><MessageCircle size={17}/> Preguntar por WhatsApp</button><div className="catalog-click-hint">Ver información completa →</div></div>
      </article>)}</div>}
      {seleccionado&&(()=>{const k=cbmCobro(seleccionado.cbm,seleccionado.peso);return <div className="catalog-detail-backdrop" onClick={()=>setSeleccionado(null)}><div className="catalog-detail" onClick={e=>e.stopPropagation()}><button className="catalog-detail-close" onClick={()=>setSeleccionado(null)} aria-label="Cerrar"><X size={22}/></button><div className="catalog-detail-head"><div className="catalog-detail-photo">{seleccionado.foto?<img src={seleccionado.foto} alt={seleccionado.nombre}/>:<div className="catalog-no-photo"><Package size={54}/><span>Imagen próximamente</span></div>}</div><div className="catalog-detail-info"><div className="catalog-code">{seleccionado.codigo}</div><h2>{seleccionado.nombre}</h2>{seleccionado.descripcion&&<div className="catalog-detail-desc">{seleccionado.descripcion}</div>}<div className="catalog-detail-metrics"><div className="catalog-detail-metric"><span>CBM por volumen</span><strong>{m3(k.vol)} m³</strong></div><div className="catalog-detail-metric"><span>CBM por peso</span><strong>{m3(k.porPeso)} m³</strong></div><div className="catalog-detail-metric"><span>CBM a cobrar</span><strong>{m3(k.cobrable)} m³</strong></div><div className="catalog-detail-metric"><span>Cobro determinado por</span><strong style={{textTransform:'capitalize'}}>{k.por}</strong></div></div><div className="catalog-detail-price"><span>Precio</span><strong>{num(seleccionado.precio_venta)>0?money(seleccionado.precio_venta):"Precio pendiente"}</strong><small style={{display:'block',fontSize:13,fontWeight:500,opacity:.78,marginTop:5}}>Precio puesto en Managua.</small></div><div style={{marginTop:20,border:'1px solid #BFE8D0',background:'#F2FFF7',borderRadius:14,padding:16}}><div style={{display:'flex',alignItems:'center',gap:8,fontWeight:900,color:'#126B3D',fontSize:18,marginBottom:8}}><MessageCircle size={22} color="#1FA463"/> ¿Tienes preguntas sobre este producto?</div><div className="text-sm" style={{color:'#49635A',marginBottom:10}}>Escribe tu consulta y envíala por WhatsApp a uno de nuestros asesores.</div><textarea className="inp" rows={3} value={consulta} onChange={e=>setConsulta(e.target.value)} placeholder="Ej. Hola, quisiera saber disponibilidad, tiempo de entrega y formas de pago."/>{whatsappDestinos.length>0?<><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}>{whatsappDestinos.map(d=><button key={d.id} className="btn" style={{background:'#1FA463',color:'#fff',fontWeight:800}} onClick={()=>abrirWhatsapp(d,seleccionado)}><MessageCircle size={16}/> Escribir a {d.name}</button>)}</div><div className="text-xs muted" style={{marginTop:8}}>Selecciona el asesor al que deseas enviar tu mensaje.</div></>:<div style={{marginTop:10,fontSize:13,color:'#A3321F'}}>En este momento no hay asesores de WhatsApp disponibles. Intenta nuevamente en unos minutos.</div>}</div></div></div></div></div>})()}
